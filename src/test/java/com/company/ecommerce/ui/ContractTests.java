...
import com.company.ecommerce.ui.pages.SignContractPage;
...
public class ContractTests extends BaseUITest {
    ...
    private SignContractPage signContractPage;
...
//        Map<String, String> expectedMap = new ObjectMapper().readValue(expectedMapString, new TypeReference<Map<String, String>>() {});
        contractListPage.searchByMap(expectedMapString);
        try {
            Thread.sleep(10000);
    ...
    @Test(dataProvider = "uicaseData", dataProviderClass = TestDataProvider.class)
    public void testSignContract(String searchMapString,String expectedMapString) throws JsonProcessingException {
        contractListPage = new ContractListPage(driver);
        // 转换为 Map<String, String>
        contractListPage.navigateTo();
        signContractPage = contractListPage.goSign(searchMapString);
        signContractPage.sign();
        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
